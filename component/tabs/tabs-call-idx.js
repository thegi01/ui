...
tabs = function(evtType, target){
	addEvent(this, evtType, function(e){
		...
		idx = Number( getDataAttr.call(t, 'idx') );
		setCurrent.call(this, idx);
	});
};
...
prevNext = function(d, n){
	...
	idx = getIdxByDirection.call(this, n);
	setCurrent.call(this, idx);
};
getIdxByDirection = function(n){
	var d = this.direction,
		idx = Number( getDataAttr.call(this, 'current') ),
		val = this.itemsLen-1;
	if( d == "prev" ){
		if(n) 
			idx = idx - n; 
...
play = function(){
	...
	el.interval = setInterval(function(){
		idx = getIdxByDirection.call(el);
		setCurrent.call(el, idx);
	}, el.timer);
};
pause = function(){
	clearInterval(this.interval);
